...
class AgregarCliente:
    def __init__(self, master,usuario):
        ...
        self.nueva_ventana.title("Registrar Cliente")
        ...
        titulo = tk.Label(self.nueva_ventana, text="Registrar Cliente", font=("Helvetica", 16))
        titulo.pack(pady=10)
        ...
        # Botón Guardar
        btn_guardar = tk.Button(frame_buttons, text="Guardar", command=self.guardar_cliente)
    ...
    def enter_pressed(self, event):
        if self.entry_cedula.get() and self.entry_nombre.get() and self.entry_apellido.get() and self.entry_telefono.get() and self.entry_email.get():  # Si los campos están llenos
            self.guardar_cliente(event)

    def guardar_cliente(self, event=None):
        ...
        mesero.AgregarCliente(cedula, nombre, apellido, telefono, email)
