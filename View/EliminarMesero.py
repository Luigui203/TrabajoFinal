...
class EliminarMesero:
    def __init__(self, master, usuario):
        ...
        # Botón Buscar
        btn_buscar = tk.Button(frame_buttons, text="Buscar", command=self.buscar_Mesero)
        btn_buscar.grid(row=0, column=0, padx=10)

        # Botón Eliminar
        btn_eliminar = tk.Button(frame_buttons, text="Eliminar", command=self.eliminar_Mesero)
    ...
    def buscar_Mesero(self):
        """Buscar un Mesero por cédula"""
        ...
        mesero = self.cajero.buscar_Mesero(cedula)

        if mesero:
            nombre, apellido, telefono, email = mesero
    ...
    def eliminar_Mesero(self):
        ...
        if self.cajero.eliminar_Mesero(cedula):
